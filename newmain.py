...
def find_PDB_only(ids_PDB, ids_Proteopedia):
    ...
    for id in proteopedia_IDS:
        if id.upper() not in PDB_IDs:
            print(id)
            i += 1
    print("Number of IDs in Proteopedia but not in PDB: " + str(i))
    i = 0
    for id in PDB_IDs:
...
def validate_PDB_IDs(list_of_IDs, protein_name):
    ...
    # if id is valid, add to valid_ids list
    valid_ids = []
...
if __name__ == "__main__":
    ...
    # get html from url
    protein_name = "actin"
    url = "https://proteopedia.org/wiki/index.php/Actin_3D_structures"
    ...
    print(f'Valid {protein_name} IDs on PDB: ' + f"{len(valid_ids)}")
    print(valid_ids)
